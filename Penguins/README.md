...
| **`.value_counts(dropna=False)`** | 📈 | Counts unique values in a Series,并明确包含缺失值 (`None`/`NaN`) 的计数。 |
...
| **`.isna().sum()`** | ➕ | Counts the total number of missing values per **column** (默认 `axis=0`)。 |
| **`.isna().sum(axis=1)`** | 📏 | Counts the number of missing values per **row**。 |
| **`.isna().mean()`** | 💯 | Calculates the **比例** (百分比) of missing values per column. |
...
| **`.dropna(subset=[...])`** | 🎯 | Removes rows that have missing values **仅**在指定的子集列中。 |
...
| **`(boolean_series == value).sum()`** | ✅ | Used to count how many elements in a Series满足某个条件（如计算缺失值为零的行数）。 |
